...
function App() {
  ...
  const [getState, setGetState] = useState('Irvine, USA'); //store location name from input field
  const [state, setState] = useState('Irvine, USA');
  //API KEY AND URL
  const apiKey = process.env.REACT_APP_API_KEY; //store API key created in .env file
  const apiUrl = `https://api.openweathermap.org/data/2.5/weather?q=${state}&appid=${apiKey}`; //store URL that calls the location from getState and also apiKey
  //side effect
  useEffect(() => {
    fetch(apiUrl)
    .then((res) => res.json())
...
<div className="App">
    ...
    <div className="container">
      ...
      <div className="card mt-3 mx-auto">
          ...
          {apiData.main
          ? (<div class="card-body text-ceter">
              <img
